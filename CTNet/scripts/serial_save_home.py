...
def main():
    ap = argparse.ArgumentParser(description="读取当前各关节 pos，并将其保存为中位(home) JSON，同时可选保存夹爪闭合位")
    ...
    ap.add_argument("--out", type=str, default="serial_home.json", help="保存路径")
    # 夹爪闭合位选项
    ...
    for i in ids:
        try:
            ...
            print(f"ID={i}: 当前 pos={pos}，将保存为中位")
