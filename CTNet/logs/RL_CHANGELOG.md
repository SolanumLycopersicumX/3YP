...
## V2.1 - Position vs Time 可视化

### 创建日期
2026-02-17

### 创建原因
- 导师反馈：需要一个 Position vs Time 图来展示系统的 overall performance
- 通过目标位置与实际位置的对比，直观展示控制精度
- 误差曲线可以量化系统的控制性能
...
| 文件 | 修改内容 |
|------|----------|
| `scripts/rl_control_test.py` | 添加轨迹记录和可视化功能 |

### 修改内容

#### 1. 新增 `RLControlResult.sample_trajectory` 字段

```python
@dataclass
class RLControlResult:
    # ... 原有字段 ...
    sample_trajectory: Optional[Dict[str, Any]] = None  # 新增
```

#### 2. 修改 `run_rl_control_episode()` 函数

**新增参数：** `record_trajectory: bool = False`

**新增记录的数据：**
- `time_steps`: 时间步列表 [0, 1, 2, ...]
- `target_y`, `target_z`: 目标位置（恒定值）
- `actual_y`, `actual_z`: 实际位置（随时间变化）
- `errors`: 位置误差 = ||actual - target||

#### 3. 新增 `visualize_position_vs_time()` 函数

**功能：** 生成 Position vs Time 图
...
- 子图 3: 位置误差曲线
  - 绿色实线：误差随时间变化
  - 红色虚线：目标阈值 (0.1)
  - 显示 Final Error 和 Mean Error 统计

#### 4. 新增 `visualize_overall_performance()` 函数

**功能：** 生成综合性能图

**输出：**
- 子图 1: 平均误差 vs 最终误差对比（各数据集）
- 子图 2: 误差收敛曲线对比（叠加显示）
- 子图 3: 综合性能指标（分类准确率、到达率、平滑度、1-误差）

### 输出文件

| 文件 | 描述 |
|------|------|
| `outputs/rl_control_test/position_vs_time.png` | Position vs Time 图 |
| `outputs/rl_control_test/overall_performance.png` | 综合性能图 |
...
┌─────────────────────────────────────────────────────────────┐
│  ┌───────────────┐  ┌───────────────┐  ┌───────────────┐   │
│  │ Y Position    │  │ Z Position    │  │ Error Curve   │   │
│  │               │  │               │  │               │   │
│  │ --- Target    │  │ --- Target    │  │ ─── Error     │   │
...
├── rl_4direction_policy.pth        # V2.2 训练模型 (4方向) ⭐
...
## V2.2 - 4方向RL模型 + 物理机械臂控制

### 创建日期
2026-02-17

### 创建原因
- 之前的 `dqn_policy_full.pth` 模型在物理机械臂测试中只会执行 "down" 动作
- 该模型未能正确学习 4 个方向 (left, right, up, down) 的控制
- 需要重新训练一个专门针对 4 方向控制的 RL 模型

### 问题分析

| 现象 | 原因 | 
|------|------|
| RL 只执行 "down" 动作 | 旧模型可能在不同的 reward 设置下训练 |
| 物理控制到达率 60% | 只有目标在 "down" 方向时才能到达 |
| left/right 方向失败 | 模型没有学习到正确的 Q 值分布 |

### 新增文件

| 文件 | 功能 |
|------|------|
| `scripts/train_rl_4direction.py` | 专门训练 4 方向 RL 模型 |
...
| Batch Size | 64 |
| Learning Rate | 3e-4 |
| ε start → end | 1.0 → 0.05 (线性衰减) |
| Target Update | Soft Update τ=0.005 |
| DQN Type | Double DQN |
...
```
[ 100/1000] reward= -0.97 reach= 19.0% ε=0.882 | L:21% R:20% U:20% D:20%
[ 200/1000] reward=  4.45 reach= 62.0% ε=0.764 | L:40% R:68% U:60% D:72%
[ 300/1000] reward=  6.66 reach= 79.0% ε=0.645 | L:84% R:80% U:72% D:84%
[ 400/1000] reward=  8.67 reach= 93.0% ε=0.526 | L:92% R:92% U:88% D:100%
[ 500/1000] reward=  9.45 reach= 98.0% ε=0.407 | L:96% R:100% U:96% D:100%
[ 600/1000] reward=  9.53 reach= 97.0% ε=0.289 | L:96% R:100% U:96% D:100%
[ 700/1000] reward=  9.99 reach= 99.0% ε=0.170 | L:100% R:100% U:96% D:100%
[ 800/1000] reward= 10.24 reach=100.0% ε=0.051 | L:100% R:100% U:100% D:100%
[ 900/1000] reward= 10.28 reach=100.0% ε=0.050 | L:100% R:100% U:100% D:100%
[1000/1000] reward= 10.28 reach=100.0% ε=0.050 | L:100% R:100% U:100% D:100%
```

### 最终评估 (100 episodes, 仿真)
...
| 指标 | 旧模型 (dqn_policy_full.pth) | 新模型 (rl_4direction_policy.pth) |
|------|------------------------------|-----------------------------------|
| 分类准确率 | 100% (5/5) | 100% (8/8) |
| 控制到达率 | 60% (3/5) ❌ | **100% (8/8)** ✅ |
| 平均步数 | 12.8 | **8.2** |
| 平均奖励 | 6.01 | **10.31** |
| left 成功 | ❌ | ✅ |
| right 成功 | ❌ | ✅ |
...
### 保存的模型
- `outputs/rl_4direction_policy.pth` (4方向 Transformer DQN)

### 关键修复

1. **PyTorch 2.6 模型加载问题**
   ```python
   # 修复前: 默认 weights_only=True 导致加载失败
   state = torch.load(model_path, map_location=device)
   
   # 修复后: 显式设置 weights_only=False
   state = torch.load(model_path, map_location=device, weights_only=False)
   ```

2. **JSON 序列化问题**
   ```python
   # 修复前: numpy.bool_ 无法 JSON 序列化
...
# 2. 测试物理机械臂
sudo chmod 666 /dev/ttyACM1  # 或已加入 dialout 组则无需此步
...
| 文件 | 描述 |
|------|------|
| `outputs/rl_4direction_policy.pth` | 训练好的 4 方向 RL 模型 |
