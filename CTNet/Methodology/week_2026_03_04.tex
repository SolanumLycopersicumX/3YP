...
\begin{frame}{Error Analysis: EEG Control Accuracy}
    \begin{columns}[T]
        \column{0.5\textwidth}
        \begin{center}
            \includegraphics[width=\textwidth]{../outputs/eeg_physical_control/error_analysis.png}
        \end{center}
        
        \column{0.5\textwidth}
        \textbf{Error Metrics:}
        \begin{itemize}
            \item Y Error: Left-right deviation
            \item Z Error: Up-down deviation
            \item Total Error: Combined deviation
        \end{itemize}
        
        \vspace{3mm}
        \textbf{Key Findings:}
        \begin{itemize}
            \item Simple patterns (S1-S4) have lower error
            \item Complex patterns (S5-S6) show higher variability
            \item Mean total error remains acceptable
        \end{itemize}
    \end{columns}
\end{frame}
