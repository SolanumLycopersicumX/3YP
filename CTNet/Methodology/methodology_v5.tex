...
This project employs two complementary datasets from the BCI Competition IV suite to validate generalisation and robustness. Table~\ref{tab:dataset_comparison} summarises their characteristics.
...
\begin{table}[t]
    \centering
    \caption{\textbf{Comparison of the two benchmark datasets.}}
    \label{tab:dataset_comparison}
    \renewcommand{\arraystretch}{1.15}
    \setlength{\tabcolsep}{4pt}
    \begin{tabular}{@{}p{0.24\columnwidth} p{0.34\columnwidth} p{0.34\columnwidth}@{}}
        \toprule
        Property & IV-2a & IV-2b \\
        \midrule
        Channels $C$ & 22 (10-20 full) & 3 (C3, Cz, C4) \\
        Classes $K$ & 4 (LH, RH, Feet, Tongue) & 2 (LH, RH) \\
        Subjects & 9 & 9 \\
        Sessions/subject & 2 & 5 \\
        Trials/session & 288 & 120 \\
        Sampling rate & 250\,Hz & 250\,Hz \\
        EOG channels & 3 & 3 \\
        \midrule
        \textbf{Strengths} & Rich spatial info; 4-class control & Multi-session; minimal hardware \\
        \textbf{Challenges} & High dimensionality & Limited spatial resolution \\
        \bottomrule
    \end{tabular}
\end{table}

\noindent\textbf{Rationale for Dual-Dataset Evaluation.}
By training and evaluating on both datasets, we address three key questions:
\begin{enumerate}
    \item \textit{Spatial Generalisation}: Does the pipeline exploit high-density spatial patterns (IV-2a) yet remain functional with minimal electrodes (IV-2b)?
