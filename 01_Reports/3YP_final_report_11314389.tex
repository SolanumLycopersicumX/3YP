...
\begin{titlepage}
    ...
    \vspace{1.5cm}
    {\Large Third Year Individual Project -- Final Report\par}
    \vspace{2.5cm}
    {\large April 2026\par}
    \vspace{2cm}
    {\large Student ID: 1131438\par}
    \vfill
    {\large Project Supervisor: Alex Casson\par}
\end{titlepage}
...
MI-based BCIs are particularly attractive because they require no external stimulation (unlike P300 or SSVEP paradigms) and rely entirely on the user's voluntary mental activity. However, translating MI-BCI systems into reliable, real-time robotic controllers remains an open problem. Three issues are especially important for this project.

EEG patterns shift across sessions, days, and subjects because electrode impedance, fatigue, and cognitive strategy are not constant~\cite{hameed2025enhancing}. A decoder calibrated on one recording session can therefore degrade when it is used in a later session or by a new user, limiting the practical lifespan of static models.

Conventional BCI pipelines also tend to operate in open loop. Each classified epoch is mapped to a fixed command, so the system has little opportunity to recover once a misclassification has occurred. In robotic manipulation, where trajectories must be smooth and correctable, a single erroneous command can disturb the rest of the motion.

Hardware constraints create a further gap between laboratory results and deployment. Many high-performing MI decoders are developed with 64-channel research-grade amplifiers, whereas a portable system may need to run on consumer-grade devices such as the 8-channel OpenBCI Cyton. Simply discarding channels risks removing the spatial information on which these decoders depend, so channel reduction has to be guided by empirical evidence about which electrodes carry the most discriminative signal.
...
Classical MI pipelines commonly extract Common Spatial Patterns (CSP) features and classify them with Linear Discriminant Analysis (LDA)~\cite{hameed2025enhancing}. CSP maximises the variance ratio between classes and produces compact spatial filters that are neurophysiologically interpretable. Its limitation is that it assumes a stable covariance structure across trials, which is often violated by session-to-session drift. CSP+LDA can therefore degrade rapidly without frequent recalibration. Filter Bank CSP (FBCSP)~\cite{ang2008fbcsp} improves robustness by applying multiple frequency sub-bands and selecting the most discriminative features, although this increases computational complexity.

Convolutional architectures relax part of this stationarity assumption by learning spatial--temporal filters directly from data. EEGNet~\cite{lawhern2018eegnet} introduced depthwise and separable convolutions for compact EEG feature extraction, achieving 68--72\% on standard 4-class benchmarks. ShallowConvNet~\cite{schirrmeister2017deep} applies temporal and spatial convolutions with log-variance pooling to capture band-power features. These models still process each trial independently, however, and do not carry information across successive epochs, which makes them susceptible to isolated noisy trials.

Recurrent networks, including LSTMs and GRUs, address this limitation by maintaining hidden states across time steps and modelling EEG dynamics sequentially. In practice, their performance on long MI epochs can be limited by vanishing gradients and sensitivity to sequence length, and their sequential computation prevents efficient parallel training~\cite{vaswani2017attention}.

Transformer-based models avoid the sequential bottleneck of RNNs by using self-attention to model pairwise dependencies in a single forward pass. EEG-Conformer~\cite{song2022eeg} combines convolutional modules with Transformer encoders for MI classification and reports state-of-the-art results. ATCNet~\cite{altaheri2022atcnet} combines attention-based temporal convolutional networks with sliding-window augmentation, while EEG-TCNet~\cite{ingolfsson2020eeg} offers a compact temporal convolutional alternative. The remaining weakness is different from that of CNNs: pure self-attention does not by itself provide the spatial inductive bias that makes EEGNet effective.

Reinforcement learning has also been applied to BCI, although mainly for offline feature selection. MARS~\cite{shin2024mars} uses multi-agent RL to optimise spatial, spectral, and temporal filter banks, and RLEEGNet~\cite{nallani2024rleegnet} formulates EEG decoding within an MDP framework. These studies show that RL can be useful in EEG pipelines, but neither closes the loop with a physical actuator.
...
The system is organised into three visual groups that correspond to four functions: EEG preprocessing, deep learning classification, reinforcement learning control, and hardware integration. In Figure~\ref{fig:system_overview}, hardware integration is shown through the BrainFlow/OpenBCI acquisition interface and the serial SO-101 control interface.
...
\end{tikzpicture}
    \end{adjustbox}
    \fi
    \caption{System architecture overview. The pipeline is shown as three visual groups spanning four functions: EEG processing, deep learning classification, reinforcement learning control, and hardware integration. Raw EEG is preprocessed, then classified by the EEGTransformer. The predicted MI class and confidence form part of the state observed by a DQN agent, whose action $a_t$ drives the robotic environment. In the control setting used here, 4-class MI predictions are mapped to four directional commands (left, right, up, down), while hardware integration is handled through the BrainFlow/OpenBCI acquisition interface and the serial SO-101 control interface.}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{plotneuralnet_eegtransformer_v13/eegtransformer_plotnn.pdf}
...
\begin{table}[htbp]
\centering
\caption{Summary of the main experimental protocols used in the project.}
\label{tab:protocol_summary}
\small
\begin{tabular}{@{} >{\raggedright\arraybackslash}p{3.0cm} >{\raggedright\arraybackslash}p{3.1cm} >{\raggedright\arraybackslash}p{3.5cm} >{\raggedright\arraybackslash}p{3.5cm} @{}}
\toprule
\textbf{Experiment} & \textbf{Data or environment} & \textbf{Optimised component} & \textbf{Evaluation output} \\
\midrule
...
Three Q-network families were compared under the same state representation so that the experiment isolated the network design rather than the environment. The CNN+LSTM baseline applies two 1D convolutions over the state sequence, followed by a single-layer LSTM and a two-layer MLP head. The full Transformer DQN projects the 5-D or 7-D state to $d_{model}=64$, adds sinusoidal or learnable positional encoding, and uses 2 Transformer encoder layers with 4 attention heads, feed-forward width 256, LayerNorm, and a Linear $64 \rightarrow 64 \rightarrow 4$ output head with GELU and dropout. The Light Transformer is a smaller alternative with $d_{model}=32$, 2 heads, one residual self-attention layer, and a shallow feed-forward block. The main RL model implementation is contained in \texttt{02\_Code/Reinforcement\_Learning/dqn\_transformer.py}, while the training and evaluation scripts define the exploration schedule, replay logic, and environment-specific reward shaping.
...
Table~\ref{tab:iv2a_results} presents per-participant classification accuracies. The EEGTransformer achieved a mean of 73.80\% $\pm$ 7.07\%, with a peak of 86.11\% on Participant~3. The OVR-CSP+LDA baseline reached 74.27\% on Participant~1 under 5-fold CV, and improved to 84.7\% when restricted to binary left/right classification, reflecting the reduced difficulty of a 2-class task.
...
Under extreme channel scarcity, the EEGTransformer achieved a mean of 82.87\% $\pm$ 8.01\% (Table~\ref{tab:iv2b_results}). The strong performance on only three electrodes confirms that the depthwise spatial convolution can extract discriminative spatial patterns even from a minimal montage, because the binary left--right task produces pronounced lateralised ERD that is detectable at C3, Cz, and C4.
...
The channel reduction study produced two complementary observations. First, classification accuracy does not decrease monotonically with channel count. For example, the 4-channel motor-cortex subset performed better than the 8-channel domain subset in the cross-subject setting. This does not imply that fewer electrodes are intrinsically better; rather, it shows that the relationship between montage size and cross-subject accuracy is affected by channel choice, subject split, and model regularisation. Second, the ablation-ranked and domain-guided subsets answer different questions. Ablation ranking asks which channels most affect the already-trained 64-channel model, whereas the domain-guided subset asks which channels are practical for an OpenBCI-oriented headset layout.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{../03_Experiments/Channel_Reduction/channel_reduction/importance/channel_importance_bar.png}
\vspace{0.6em}

\includegraphics[width=0.62\textwidth]{../03_Experiments/Channel_Reduction/channel_reduction/importance/channel_importance_topo.png}
\caption{Ablation-based channel importance from the trained 64-channel PhysioNet model. The upper bar plot ranks the channels whose removal most affected held-out accuracy, while the lower topographic plot shows the spatial distribution of the same analysis.}
\label{fig:channel_importance}
\end{figure}

Figure~\ref{fig:channel_importance} explains why C3 appears repeatedly in the reduced-channel analysis. C3 lies over the left sensorimotor cortex and is expected to be informative for right-hand motor imagery, so its high ablation importance is neurophysiologically plausible. Other selected electrodes, such as CP3, FCz, and FC2, are also close to motor or premotor regions. At the same time, the appearance of Fpz, O1, and F8 in the purely ablation-ranked top-8 subset is a useful caution: an empirical ranking can capture dataset-specific correlations or artefact structure as well as genuine motor-cortex activity. This is why the final OpenBCI-oriented 8-channel subset was not chosen solely by the ablation list, but was constrained toward a motor-cortex montage.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.92\textwidth]{../03_Experiments/Channel_Reduction/channel_reduction/channel_reduction_comparison.png}
\caption{Cross-subject PhysioNet accuracy under progressive channel reduction. The comparison separates domain-guided motor-cortex subsets from ablation-ranked subsets, showing that channel identity matters as much as channel count.}
\label{fig:channel_reduction_comparison}
\end{figure}

The 8-channel motor cortex configuration (C3, C4, FC3, FC4, CP3, CP4, Cz, FCz), designed as an OpenBCI Cyton-compatible electrode set for offline evaluation, achieved 40.7\% in cross-subject mode. Applying subject-specific fine-tuning recovered much of the lost performance, reaching 72.54\% on average (Table~\ref{tab:8ch_finetune}). This confirms that transfer learning can partially compensate for reduced spatial coverage by adapting the model to individual neural patterns. In the current online BrainFlow pipeline, however, low-channel hardware streams are still adapted to the legacy CTNet input shape before inference, so this result should be interpreted as evidence that an 8-channel montage is viable offline rather than as proof of a native end-to-end 8-channel deployment.
...
Figure~\ref{fig:finetune_8ch_comparison} shows the corresponding 8-channel fine-tuning behaviour. S7 and S48 remain close to their 64-channel performance, while S70 and S50 degrade more severely. This participant dependence suggests that channel reduction should eventually be personalised as well: a fixed OpenBCI montage is attractive for setup simplicity, but some users may require a slightly different electrode subset or a longer calibration session to reach stable control performance.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.78\textwidth]{../03_Experiments/physionet_ctnet_finetune_8ch/finetune_comparison.png}
\caption{Participant-specific fine-tuning accuracy for the OpenBCI-compatible 8-channel model. The mean accuracy is 72.54\%, but the spread across participants shows that reduced-channel deployment is strongly user-dependent.}
\label{fig:finetune_8ch_comparison}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../03_Experiments/architecture_comparison_v2/comparison_v2.png}
\caption{RL architecture comparison on the 2D reaching task. The figure complements Table~\ref{tab:dqn_comparison} by showing the trade-off between reach rate, reward, parameter count, and training time for the CNN+LSTM, Light Transformer, and Transformer DQN agents.}
\label{fig:rl_architecture_comparison}
\end{figure}
...
Earlier development experiments in the source CTNet project also tested 8-direction and adaptive-step variants. These were useful for understanding smooth robotic movement, but the final report uses the 4-direction setting as the main quantitative benchmark because it aligns directly with the four MI classes used by PhysioNet: left, right, both fists/up, and both feet/down. Figures~\ref{fig:adaptive_step_comparison} and~\ref{fig:adaptive_step_trajectory} give the most relevant supporting result: for Subjects 1, 3, 5, and 6, the selected-subject reconstruction reduced total movement from 165 to 87 control steps while preserving target reach, and the archived spatial comparison shows cleaner adaptive trajectories. This is interpreted as faster trajectory convergence, not as an EEG decoding improvement.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.80\textwidth]{../03_Experiments/comparison_selected_subjects_fixed_vs_adaptive.png}
\caption{Fixed-step versus adaptive-step sequence-control comparison for Subjects 1, 3, 5, and 6. The step-count trajectories were regenerated from the deterministic target sequences and fixed/adaptive step-size rules used to summarise the archived development experiment; adaptive stepping preserves target reach while reducing the number of control steps in each displayed sequence.}
\label{fig:adaptive_step_comparison}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\textwidth]{../03_Experiments/comparison_selected_subjects_fixed_vs_adaptive_trajectory.png}
\caption{Selected spatial trajectories extracted from the archived all-subject fixed-step/adaptive-step comparison for Subjects 1, 3, 5, and 6. Target regions are shown as shaded circles; the fixed-step panels retain the visibly less stable paths from the archived development run, while the adaptive-step panels show cleaner movement toward the same targets.}
\label{fig:adaptive_step_trajectory}
\end{figure}
...
\subsubsection{Software-Interface and BrainFlow Validation}
\label{sec:brainflow_results}
...
\begin{table}[htbp]
\centering
\caption{BrainFlow / physical-control interface validation using synthetic-board streaming and simulated arm motion.}
\label{tab:brainflow_validation}
\small
\begin{tabular}{@{} >{\raggedright\arraybackslash}p{3.0cm} >{\raggedright\arraybackslash}p{3.4cm} c >{\raggedright\arraybackslash}p{4.4cm} @{}}
\toprule
\textbf{Validation item} & \textbf{Setting} & \textbf{Value} & \textbf{Interpretation} \\
\midrule
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../03_Experiments/brainflow_physical_control/all_pos_vs_time.png}
\caption{BrainFlow synthetic-board interface validation across six movement patterns. The figure records position over time for the archived online-control runs and verifies repeated closed-loop execution through the software interface.}
\label{fig:brainflow_validation}
\end{figure}

The BrainFlow validation should not be over-interpreted as a live BCI experiment. Its value is narrower: it verifies that the software interface can pass data through the same modules that a live experiment would use, reducing future integration risk once ethical approval and live signal collection are available.
...
The EEGTransformer's competitive performance across three datasets is consistent with its two-stage design. The convolutional front-end supplies spatial inductive bias through data-driven filters similar in role to CSP, while the Transformer encoder models temporal relationships across the full offline PhysioNet epoch. This interpretation is supported by the results: on BCI IV-2a, the model's 73.80\% mean exceeds typical EEGNet and FBCSP ranges~\cite{hameed2025enhancing}, and on IV-2b it still reaches 82.87\% with only three channels.

\subsubsection{Transfer Learning Bridges the Cross-Subject Gap}

The 56.54\% cross-subject accuracy on the 109-participant PhysioNet dataset reflects the well-documented challenge of EEG generalisation. Inter-individual differences in anatomy, skull conductivity, electrode placement, and MI strategy create distribution shifts that no single pooled model can fully absorb. The improvement to 88.78\% after fine-tuning suggests that the cross-subject model learns transferable MI structure and then adapts it to each user's signal distribution using a relatively small calibration set.

For deployment, this transfer-learning approach may reduce calibration burden compared with training a classifier from scratch for each user, because the pre-trained model provides a stronger initialisation. A personalised model can then be reused in later sessions, although recalibration may still be needed after substantial electrode-placement changes.
...
The 8-channel fine-tuned accuracy of 72.54\% is lower than the 64-channel result but remains well above the 25\% chance level for 4-class classification. Together with the end-to-end results in Section~\ref{sec:e2e_results}, this suggests that the 8-channel set is a plausible offline foundation for closed-loop control, while real-time OpenBCI validation remains future work.
...
The filter ablation study (Section~\ref{sec:ica_results}) supports the 8--30\,Hz bandpass filter used in the main motor-imagery pipeline. Filtering improved accuracy by $+18.44\%$ on average across 5 participants, consistent with retaining Mu and Beta rhythms while removing slow drift, mains noise, and some muscle artefact.

In contrast, adding ICA on top of bandpass filtering improved accuracy by only $+1.51\%$ on average. The participant-dependent effect (S7 improved by 5.56\%, S1 degraded by 2.78\%) suggests that a fixed ICA policy can remove useful signal as well as artefact, so the bandpass-only pipeline was retained.

\subsubsection{Current Hardware Validation Status}

At the time of writing, all core software components have been implemented and validated at dataset, simulation, or software-interface level. The EEGTransformer was evaluated on BCI IV-2a, IV-2b, and PhysioNet; the DQN controller was tested in PyBullet; the BrainFlow path was exercised with synthetic-board streaming; and the SO-101 serial protocol was verified with basic motion commands.

Live human subject testing was not performed because this undergraduate project does not have University of Manchester ethical approval for human experimentation. The end-to-end evaluation therefore simulates real-time use by feeding recorded PhysioNet EEG through the classifier and using PyBullet for robotic arm control. This supports technical feasibility, while live-subject validation remains future work.

\subsubsection{Limitations}
\label{sec:disc_limitations}

Several limitations should be acknowledged. Subject-specific fine-tuning was validated on 10 of 109 PhysioNet subjects due to computational constraints, so full-cohort validation would strengthen generalisability. Fine-tuning also introduces a per-user calibration step that future online adaptation should reduce. The 8-channel result remains offline evidence and must be validated with OpenBCI hardware and new participants under appropriate ethical approval. Finally, the 4-second online-control window may be too slow for time-critical use, motivating shorter windows or asynchronous MI detection.
...
Project management was integrated into the technical workflow. The Gantt chart divided the work into dataset preparation, baseline reproduction, EEGTransformer development, transfer learning, channel reduction, RL control, and hardware integration, so later stages depended on validated outputs. Weekly supervision, interim drafts, version-controlled code, and saved experiment outputs shaped the scope: early 9-subject results motivated the 109-participant PhysioNet evaluation, and the OpenBCI target made channel reduction a core work package.

Risk management combined technical contingency planning with the formal University health-and-safety process. Control complexity, cross-subject generalisation, and reproducibility risks were mitigated through a planar reaching task, participant-specific fine-tuning, and preserved scripts, models, and outputs. The formal risk assessment covered electrical equipment, robotic-arm movement, pinch or impact hazards, and unexpected BCI-driven motion, with controls including a clear operating zone, reduced-speed physical tests, workspace and velocity limits, warning before movement, and immediate power-off access. The budget was limited to the \pounds178.20 SO-101 robotic arm kit; supporting planning, risk, CPD, and repository artefacts are listed in the appendices.
...
If this project were handed to a new student next year, the first recommendation would be an ethically approved live-user study using the 8-channel montage identified here. The second recommendation is a native real-time 8-channel inference path trained with the exact OpenBCI channel order, sampling rate, filtering, normalisation, and electrode-placement constraints expected during deployment.
...
\bibitem{lee2019openbmi}
M.-H.~Lee \emph{et al.}, ``EEG dataset and OpenBMI toolbox for three BCI paradigms: an investigation into BCI illiteracy,'' \textit{GigaScience}, vol.~8, no.~5, 2019.

\bibitem{bnci2020datasets}
BNCI Horizon 2020, ``Data sets: 001-2014 four class motor imagery and 002-2014 two class motor imagery,'' online database, accessed 2026.
...
Appendix~A contains the preliminary project proposal submitted earlier in the academic year. The repository copy is stored as \texttt{05\_Documentation/11314389\_v4.pdf}. It is included here as the original approved proposal document and shows the initial scope against which the completed project can be compared.

\includepdf[pages=-,pagecommand={}]{../05_Documentation/11314389_v4.pdf}

\section{Project Plan and Gantt Chart}

Appendix~B contains the final updated project plan and Gantt chart used at the end of the project. The repository copy is stored as \texttt{05\_Documentation/gantt\_chart\_v3.pdf}. This artefact provides supporting evidence for the planning decisions summarised in Chapter~5.

\includepdf[pages=-,pagecommand={}]{../05_Documentation/gantt_chart_v3.pdf}

\section{Risk Register and Health \& Safety Artefact}

Appendix~C contains the maintained risk register together with the formal health-and-safety documentation for the project. The repository copies of the risk and health-and-safety assessment are stored as \texttt{05\_Documentation/Risk assessment form-11314389\_v3.docx} and \texttt{05\_Documentation/Risk assessment form-11314389\_v3.pdf}. These materials support the risk-management discussion in Chapter~5.

\includepdf[pages=-,pagecommand={}]{../05_Documentation/Risk assessment form-11314389_v3.pdf}

\section{Continuing Professional Development (CPD) Log}

Appendix~D contains the CPD log maintained during the project. It records the self-directed learning activities that supported the implementation work, including study in MNE-Python, PyTorch, Transformer models, reinforcement learning, and BrainFlow integration. The repository also contains dated progress material that complements this record, including \texttt{01\_Reports/week\_2026\_03\_24.tex}.

\includepdf[pages=-,pagecommand={}]{../05_Documentation/Zheng_Xu_11314389_CPD_Log.pdf}
