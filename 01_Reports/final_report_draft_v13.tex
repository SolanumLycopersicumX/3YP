...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.78\textwidth]{../03_Experiments/Channel_Reduction/channel_reduction/channel_reduction_comparison.png}
...
Project management formed part of the main project workflow throughout the year rather than being treated as a separate administrative task. An initial Gantt chart divided the work into sequential but partly overlapping packages: dataset preparation, reproduction of baseline classification pipelines, development of the hybrid EEGTransformer, transfer-learning experiments, channel reduction, RL control design, and finally hardware integration. This ordering was intentional. It reduced project risk by ensuring that later stages depended on earlier validated outputs rather than on untested assumptions. For example, the RL stage was postponed until a stable classification interface had been defined, and hardware-oriented work was not treated as the main validation route because ethical approval constraints made a simulation-first strategy more realistic.

Progress was monitored through weekly supervision meetings, interim report drafts, and version-controlled code and experiment outputs. These checkpoints were not only administrative; they directly affected technical decisions. When early results showed that small 9-subject datasets were insufficient to support a strong cross-subject argument, the project scope was revised to include the 109-participant PhysioNet evaluation. Likewise, when OpenBCI deployment emerged as an important practical target, the channel-reduction study was elevated from an optional extension to a core work package. In this sense, scope change was managed through evidence rather than through ad hoc feature expansion: new tasks were added only when they strengthened the main research question.

Risk management was handled continuously alongside technical work. A formal health-and-safety assessment was maintained for the hardware aspects of the project, while technical risks were tracked through contingency planning. One major risk concerned the complexity of robotic control: a full high-DOF manipulation setting would have consumed disproportionate development time, so a simplified planar reaching environment was adopted for the main RL comparison. A second risk concerned dataset heterogeneity and weak cross-subject generalisation; this was mitigated by adding transfer learning and participant-specific fine-tuning rather than relying only on pooled models. A third risk concerned reproducibility, addressed through structured experiment scripts, saved model artefacts, and preservation of outputs under the repository's experiment directories.

Health and safety was managed through the University risk-assessment process rather than left to informal judgement. The maintained assessment covered prolonged display-screen work, general electrical equipment, lone working, fire risk, software/data loss, robotic-arm electrical setup, mechanical pinch or impact hazards, object displacement, and unexpected BCI-driven arm motion. The main controls were ergonomic workstation practice and regular breaks; visual inspection of cables and power supplies; keeping liquids away from electrical equipment; maintaining an unobstructed operating zone around the arm; reduced-speed physical tests; software limits on workspace, velocity, and joint range; clear warning before movement; a minimum 0.5\,m observer distance; and immediate access to a power-off emergency stop. Because live human EEG testing was outside the approved project scope, the BCI evaluation used public datasets and simulation, which substantially reduced participant and hardware risk. The formal artefact is stored as \texttt{05\_Documentation/Risk assessment form-11314389\_v3.pdf}.

The project budget was intentionally kept small by using public datasets, open-source software, and existing personal computing equipment. The only project-specific purchase was an SO-101 robotic arm kit from AliExpress, costing \pounds178.20. This kit included the 3D-printed arm components, servo motors, bench vice, controller/electronics board, and communication cables required for physical setup and serial-control testing. Model training was carried out on the author's own desktop computer with an Intel i9-13900K CPU and NVIDIA RTX 4070 Ti GPU; because this machine was already personally owned before the project, it is not counted as a project budget item.

\begin{table}[htbp]
\centering
\caption{Major project deliverables and the evidence retained for assessment and reproducibility.}
\label{tab:deliverable_evidence}
\small
\begin{tabular}{@{} >{\raggedright\arraybackslash}p{3.2cm} >{\raggedright\arraybackslash}p{4.6cm} >{\raggedright\arraybackslash}p{5.0cm} @{}}
\toprule
\textbf{Work package} & \textbf{Deliverable produced} & \textbf{Evidence retained} \\
\midrule
Classifier development & EEGTransformer adapted to 22-, 3-, 64-, and reduced-channel inputs & Training scripts, saved model outputs, benchmark tables, and architecture diagram \\
Transfer learning & Pooled PhysioNet model plus participant-specific fine-tuning workflow & Fine-tuning summaries, per-subject fold results, and comparison figure \\
Channel reduction & Ranked electrodes and OpenBCI-oriented 8-channel configuration & Ablation outputs, channel-importance plots, topographic visualisation, and reduced-channel accuracy comparison \\
Controller design & Transformer and Light Transformer DQN policies for planar reaching & Architecture-comparison logs, reward/reach metrics, and saved policy artefacts \\
End-to-end integration & EEG classification output connected to simulated robotic control & Multi-subject sequence logs and comparison between state-only and EEG-augmented control \\
Hardware pathway & BrainFlow acquisition wrapper and SO-101 serial-control interface & Synthetic-board validation logs, position traces, and risk controls for future physical testing \\
Project governance & Planning, risk assessment, health-and-safety record, and CPD evidence & Gantt chart, risk register, health-and-safety artefact, CPD log, and repository README \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:deliverable_evidence} shows how the project was made auditable. Each work package produced both an implemented artefact and a retained evidence trail. This was important because the report could otherwise understate the amount of engineering work behind the final figures. For example, the channel-reduction result is not only a single 8-channel accuracy value; it is supported by ablation rankings, domain-guided electrode selection, topographic inspection, and a separate fine-tuning recovery experiment. Similarly, the hardware pathway is not presented as a completed live-user BCI, but it is supported by interface-level checks that make the remaining ethical and experimental steps clearer for a future continuation project.
...
This project developed a complete BCI-to-robot control pipeline that addresses three longstanding challenges in the field. The EEGTransformer combines convolutional spatial filtering with Transformer-based temporal modelling, achieving competitive accuracies across three datasets (73.80\% on IV-2a, 82.87\% on IV-2b, and 88.78\% on PhysioNet with fine-tuning) and confirming that the hybrid architecture generalises across electrode densities, class counts, and subject populations. Transfer learning via subject-specific fine-tuning bridged the 32-percentage-point cross-subject generalisation gap with minimal calibration data. A systematic channel reduction study established that an offline 8-channel configuration compatible with the OpenBCI Cyton retains 72.54\% accuracy after fine-tuning, identifying C3 as the most critical motor cortex electrode. An end-to-end evaluation demonstrated that the Transformer DQN achieves 99\% control success even when driven by 82\%-accurate EEG classifications, directly quantifying the value of closing the control loop. A controlled ICA comparison showed only marginal preprocessing benefit ($+1.51\%$), leading to an empirically motivated decision to adopt a bandpass-only pipeline.

All software components---EEG preprocessing, deep learning classification, reinforcement learning control, and simulated robotic arm control---have been implemented and validated. The EEG components were evaluated offline on pre-recorded datasets, while the control components were evaluated in PyBullet simulation. Future work with appropriate ethical clearance would validate the closed-loop system with live human subjects and physical hardware.

%-----------------------------------------------------------------------------
\subsection{Future Work}
%-----------------------------------------------------------------------------

If this project were handed to a new student next year, the first recommendation would be to turn the current simulation-first pipeline into an ethically approved live-user study. That would involve preparing an experimental protocol, participant information sheet, consent form, and risk assessment for OpenBCI data collection, then collecting a small but well-controlled calibration dataset using the 8-channel montage identified here. This would directly test whether the offline channel-reduction result transfers to real low-cost hardware.

The second recommendation would be to build a native real-time 8-channel inference path rather than adapting the 64-channel PhysioNet workflow after the fact. The next version should train and calibrate models using the exact OpenBCI channel order, sampling rate, filtering, normalisation, and electrode-placement constraints expected during deployment. This would reduce the train--deployment mismatch and make the system easier to reproduce by another user.

The third recommendation would be to reduce command latency. The present design uses a 4.0\,s online window because it matches the 1000-sample CTNet input convention, but a practical robot controller would benefit from shorter overlapping windows, confidence-based rejection, and asynchronous MI detection. A useful continuation would compare 1\,s, 2\,s, and 4\,s windows under the same classifier and RL policy to quantify the accuracy--latency trade-off.

Finally, future work should extend the controller from the current planar reaching abstraction toward richer SO-101 manipulation. A natural progression is to retain the present 4-direction policy as a safety baseline, then add 8-direction or 6-DOF actions only after the live EEG decoder and emergency-stop controls have been validated. Online adaptive fine-tuning should also be investigated so that the model can update to each user's current EEG distribution without requiring a full recalibration session.
