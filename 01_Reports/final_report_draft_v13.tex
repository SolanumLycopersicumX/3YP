...
\titlespacing*{\subsection}{0pt}{0.9ex plus .2ex minus .2ex}{0.45ex plus .1ex}
\titlespacing*{\subsubsection}{0pt}{0.7ex plus .2ex minus .2ex}{0.35ex plus .1ex}
...
The project builds on the general BCI-to-robot-control theme of earlier departmental work~\cite{prev_student2024}, but shifts the technical emphasis toward large-scale PhysioNet validation, transfer learning, channel reduction, and closed-loop reinforcement-learning control.
...
\begin{figure}[htbp]
    \centering
    \begin{adjustbox}{max width=1.15\textwidth,max totalheight=0.72\textheight,center}
...
The 8-channel motor cortex configuration (C3, C4, FC3, FC4, CP3, CP4, Cz, FCz), designed as an OpenBCI Cyton-compatible electrode set for offline evaluation, achieved 40.7\% in cross-subject mode. Applying subject-specific fine-tuning recovered much of the lost performance, reaching \textbf{72.54\%} on average (Table~\ref{tab:8ch_finetune}). This confirms that transfer learning can partially compensate for reduced spatial coverage by adapting the model to individual neural patterns. In the current online BrainFlow pipeline, however, low-channel hardware streams are still adapted to the legacy CTNet input shape before inference, so this result should be interpreted as evidence that an 8-channel montage is viable offline rather than as proof of a native end-to-end 8-channel deployment.
...
The following pages include the preliminary project proposal submitted earlier in the academic year. This artefact shows the original approved scope against which the completed project can be compared.
\includepdf[pages=-,pagecommand={\thispagestyle{plain}}]{../05_Documentation/11314389_v4.pdf}

\section{Project Plan and Gantt Chart}

The following page includes the final updated project plan and Gantt chart used at the end of the project. The chart records planned and actual timing, percentage completion, and the remaining final demonstration activity. It provides supporting evidence for the planning decisions summarised in Chapter~5.
\includepdf[pages=-,pagecommand={\thispagestyle{plain}}]{../05_Documentation/gantt_chart_v3.pdf}

\section{Risk Register}

Table~\ref{tab:appendix_risk_register} records the final technical risk register used to manage project delivery. The separate formal Health \& Safety Risk Assessment is included in Appendix~D.

\begingroup
\footnotesize
\setlength{\tabcolsep}{3pt}
\renewcommand{\arraystretch}{1.2}
\begin{longtable}{@{}p{2.8cm}p{3.2cm}p{3.4cm}p{4.0cm}p{1.9cm}@{}}
\caption{Final technical risk register.}
\label{tab:appendix_risk_register}\\
\toprule
\textbf{Risk} & \textbf{Cause / trigger} & \textbf{Impact} & \textbf{Mitigation / response} & \textbf{Final status} \\
\midrule
\endfirsthead
\toprule
\textbf{Risk} & \textbf{Cause / trigger} & \textbf{Impact} & \textbf{Mitigation / response} & \textbf{Final status} \\
\midrule
\endhead
RL training instability & Sparse rewards, noisy EEG-derived command evidence, or unstable Q-learning updates. & Controller may fail to learn reliable target-reaching behaviour. & Used a simplified planar reaching environment, shaped rewards, Double DQN updates, replay buffer, target-network updates, and architecture comparison before final evaluation. & Controlled \\
Weak cross-subject EEG generalisation & High inter-subject variability in MI-EEG patterns and electrode responses. & Classifier accuracy may not support meaningful control claims. & Added large-scale PhysioNet pooled training and participant-specific fine-tuning; reported both cross-subject and fine-tuned results. & Controlled \\
Reduced-channel performance loss & Moving from 64-channel laboratory data toward an 8-channel OpenBCI-compatible montage. & Low-cost deployment claim could be unsupported. & Ran leave-one-channel-out importance analysis, progressive channel reduction, and 8-channel fine-tuning before selecting the deployment-oriented montage. & Controlled \\
Preprocessing overclaim & Assuming ICA or filtering choices help without controlled evidence. & Report conclusions could be methodologically weak. & Added controlled filter/no-filter and filter/ICA comparisons; retained the bandpass-only pipeline because the measured ICA gain was small. & Controlled \\
Simulation-to-real mismatch & PyBullet and offline EEG data cannot reproduce all physical-arm and live-user conditions. & Physical deployment claims could be overstated. & Explicitly limited claims to offline and simulation validation; kept live-user and full physical validation as future work requiring ethical approval. & Controlled \\
Human-subject ethics constraint & No approval for collecting live EEG from participants. & Live BCI validation cannot be performed within the project. & Used public datasets, synthetic BrainFlow tests, and simulation-based control evaluation; stated the limitation in the abstract, discussion, and conclusion. & Controlled \\
Reproducibility and data-loss risk & Multiple scripts, checkpoints, plots, and report versions could diverge or be lost. & Markers may be unable to inspect the software contribution or verify results. & Preserved code in Git, maintained saved experiment outputs, organised the repository, and provided README and repository links in Appendix~F. & Controlled \\
Hardware safety risk during arm tests & Unexpected arm motion, pinch points, workspace collision, or electrical faults. & Injury or equipment damage during physical testing. & Used simulation-first development, velocity/workspace limits, home/return poses, operating-zone controls, and a formal H\&S assessment. & Controlled \\
\bottomrule
\end{longtable}
\endgroup
...
The following pages include the formal Health \& Safety Risk Assessment for software development, simulation work, and limited robotic-arm setup/testing.
\includepdf[pages=-,pagecommand={\thispagestyle{plain}}]{../05_Documentation/Risk assessment form-11314389_v3.pdf}

\section{Continuing Professional Development (CPD) Log}

The following tables reproduce the CPD log maintained during the project. They record the planned and executed self-learning that supported the implementation work.

\begingroup
\footnotesize
\setlength{\tabcolsep}{3pt}
\renewcommand{\arraystretch}{1.18}
\begin{longtable}{@{}p{3.1cm}p{8.4cm}p{2.4cm}p{1.2cm}@{}}
\caption{Current and recent CPD activity.}
\label{tab:appendix_cpd_current}\\
\toprule
\textbf{CPD activity title} & \textbf{Description} & \textbf{Dates} & \textbf{Hours} \\
\midrule
\endfirsthead
\toprule
\textbf{CPD activity title} & \textbf{Description} & \textbf{Dates} & \textbf{Hours} \\
\midrule
\endhead
Project planning and CPD setup & Reviewed the EEEN30330 handbook requirements for weekly project meetings, progress review meetings, project-logbook evidence, self-directed learning, and skill development. Set up the project plan, risk tracking, and a habit of recording supervisor actions and technical progress. & 06 Oct--20 Oct 2025 & 6 \\
MI-BCI literature review & Studied motor imagery BCI literature, CTNet/EEGTransformer approaches, EEGNet, CSP-based baselines, and reinforcement-learning BCI work. This developed the background needed to justify the hybrid classifier and closed-loop controller choices in the report. & Oct--Nov 2025 & 16 \\
EEG preprocessing with MNE-Python & Learned how to load PhysioNet and BCI Competition data, map labels, epoch motor-imagery trials, apply the 8--30\,Hz bandpass filter, normalise data, and maintain participant-wise train/test splits. & Nov--Dec 2025 & 18 \\
PyTorch CTNet and benchmark evaluation & Implemented and adapted the CTNet/EEGTransformer model in PyTorch, including training loops, evaluation reports, and saved checkpoints. Benchmarks were run on BCI IV-2a, BCI IV-2b, and the larger PhysioNet motor-imagery dataset. & Dec 2025--Jan 2026 & 24 \\
Transfer learning and channel reduction & Developed subject-specific fine-tuning, filter/ICA comparisons, and channel-reduction experiments. The work linked technical decisions to deployment constraints, including an OpenBCI-oriented 8-channel setup and the observed importance of motor-cortex channels such as C3. & Feb--Mar 2026 & 28 \\
RL control and simulation demos & Studied DQN control, reward shaping, Gym/PyBullet simulation, and trajectory diagnostics. Reviewed the available demonstration media: \texttt{gym\_arm.gif} as a 500x500, 10-frame, 5\,s simulation demo, and \texttt{rl\_arm.gif} in both experiment and documentation folders as identical 500x500, 20-frame, 10\,s RL trajectory animations. & Mar--Apr 2026 & 22 \\
BrainFlow, OpenBCI and SO-101 integration & Learned the BrainFlow acquisition workflow, synthetic-board testing, online buffer handling, classifier-to-policy action mapping, and serial control of the SO-101 arm. Safety-related learning included workspace limits, velocity limiting, clear operating zones, and immediate power-off access. & Apr 2026 & 16 \\
Report, repository and presentation preparation & Prepared final-report evidence, repository structure, README material, figures, and appendices. This activity focused on academic integrity, clear referencing, traceable software artefacts, and a concise technical narrative for the final demonstration/presentation. & Apr--May 2026 & 14 \\
\bottomrule
\end{longtable}

\begin{longtable}{@{}p{3.1cm}p{6.7cm}p{3.5cm}p{2.1cm}@{}}
\caption{Planned and future CPD activity.}
\label{tab:appendix_cpd_future}\\
\toprule
...
\midrule
\endhead
Final demonstration rehearsal & Practise the 15-minute final presentation and rehearse responses to questions about classifier accuracy, simulation-only validation, ethical constraints, and the RL demonstration media. & Technical communication; evidence-based explanation; time management. & 05 May--15 May 2026 \\
Repository release and reproducibility check & Clean the public repository evidence so that a technically competent reader can inspect the code structure, run key scripts, and understand which artefacts are authored work versus third-party reference material. & Version control; reproducible research; professional software documentation. & May 2026 \\
Ethically approved OpenBCI user study & Prepare a live-user validation plan with participant information, consent, risk controls, and a small 8-channel calibration protocol before collecting any new human EEG data. & Research ethics; participant protocol design; EEG acquisition practice. & Summer--Autumn 2026 \\
Real-time 8-channel inference path & Build and test a native low-channel online decoder using the exact OpenBCI channel order, sampling rate, filtering, and normalisation expected during deployment. Compare shorter decision windows to reduce command latency. & Online signal processing; model adaptation; latency analysis. & Summer 2026 \\
Extended SO-101 closed-loop manipulation & Extend the present planar reaching controller toward richer SO-101 tasks only after validating live decoding and emergency-stop controls. Use the current 4-direction policy as the safety baseline. & Robotics control; reinforcement-learning generalisation; safety validation. & Future MEng / research work \\
\bottomrule
\end{longtable}
\endgroup
...
This appendix collects selected software-interface plots that were not placed in the main body because the main report focuses on aggregate findings. The figures below provide additional engineering evidence that the BrainFlow acquisition wrapper, classifier interface, command mapping, and simulated robot-control loop can execute interpretable guided trajectories. These plots do not represent live human EEG validation.

\subsection{Additional Software-Interface Control Trajectories}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth,height=0.72\textheight,keepaspectratio]{../03_Experiments/brainflow_physical_control/a_horizontal.png}
\caption{Guided BrainFlow-to-CTNet-to-robot software-interface validation for a horizontal left--right--centre trajectory. The plot records position over time, the 2D path, classifier confidence traces, and the command distribution for a successful guided run.}
\label{fig:appendix_brainflow_horizontal}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth,height=0.72\textheight,keepaspectratio]{../03_Experiments/brainflow_physical_control/e_square_cw.png}
\caption{Guided BrainFlow-to-CTNet-to-robot software-interface validation for a clockwise square trajectory. This figure is included as engineering evidence for the acquisition/classifier/control interface only; it does not represent live human EEG validation.}
\label{fig:appendix_brainflow_square}
\end{figure}
