...
REFLECTIVE_QA = [
    ("What was the most challenging part of the project?", "The hardest part was integrating offline EEG decoding with a closed-loop control task while keeping the validation claims honest. It was easy to make a classifier or a controller in isolation, but harder to connect them without overclaiming live BCI performance."),
    ...
    ("What is the weakest part of the project?", "The weakest part is the lack of live human-subject OpenBCI validation. The project supports feasibility and integration readiness, but it does not prove a working real-time human-controlled robot."),
    ("Which result would you trust least?", "I would treat the 8-channel result cautiously because it is offline and user-dependent. It is promising, but real hardware, electrode placement, and live artefacts may change performance."),
    ("What would you do next?", "First, obtain ethical approval and run a live-user OpenBCI study using the 8-channel montage. Second, build a native 8-channel real-time inference path instead of adapting from the 64-channel pipeline."),
    ...
    ("If you restarted the project, what would you do differently?", "I would define the live-hardware validation route earlier and benchmark native 8-channel training sooner. That would reduce the gap between offline feasibility and deployment validation."),
    ("What is the main risk of overclaiming?", "The main risk is saying the system proves real-time human BCI robotic control. The correct claim is offline decoding, simulated closed-loop control, and software-interface readiness."),
    ("Why is the project still valuable without live testing?", "Because it identifies and tests the main system components needed for deployment: transferable decoding, channel reduction, controller robustness, and software integration boundaries."),
    ("What is the one-sentence conclusion?", "The project shows that EEGTransformer decoding, subject adaptation, OpenBCI-oriented channel reduction, and DQN closed-loop control can form a feasible MI-BCI robotic-control pipeline, but live OpenBCI validation remains future work."),
]
